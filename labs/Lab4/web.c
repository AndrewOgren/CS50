...
/* 
 * SaveWebPage - save the html of the webpage along with its url and depth in a file
 * 
 * Assumptions: the webpage is properly allocated with its url, html, and depth
 * fileNum is an integer
 * directPath is the path to the directory that the file is to be written to
 * directPath should end in a '/'
 * 
 * Pseudocode: 
 * Get the url, html, and depth
 * Convert the fileNum to a string
 * concatenate the directPath and fileNum into the full path to the file
 * open that file, and make sure it can be written to
 * Write the url, depth, and html to the file
 * Close the file, and free memory as appropriate
 */

int SaveWebPage(WebPage* page, int* fileNum, char* directPath){
    int file= *fileNum;
    ...
    // Properly convert the depth to a string
    int intLength = snprintf(NULL, 0, "%d", depth);
    char *depthString= malloc(intLength + 1);
    ...
    int numChars= strlen(directPath) + 1;
    int numInts= snprintf(NULL, 0, "%d", file);
    // Create path to filename
    char *fileName=  malloc((numInts + 1) + (sizeof(char)*numChars)); 
    if (fileName == NULL){
        free(depthString);
        return 1;
    }
    else {
        sprintf(fileName, "%s%d", directPath, file);
    }
    // Open file
    FILE *fp;
    fp = fopen(fileName, "w");
    // If opening file fails, close it, free memory, return
    if (fp == NULL){
        fclose(fp);
        free(depthString);
        free(fileName);
    ...
    // Append the appropriate information to the file
    else {
        fputs(URL, fp); 
        fputs("\n", fp);
    ...
    // Close the file and free appropriate memory
    fclose(fp);
    free(depthString);
    free(fileName);
    return 0;
