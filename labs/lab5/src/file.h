 ...
 *
 * Returns the newly created htmlString, which contains the html of the file
 */
char *LoadDocument(const char *directory, char *fileName);

/*
 * GetDocumentId - Creates a unique document id for each file
 * @fileName - Gthe name of the file to get the id for
 * 
 * Returns the id, or else -1 if the id creation fails
 */
